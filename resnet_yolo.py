...
valid_annot_folder = 'F:\\zzx\\YOLO\yolo_resnet\\basic-yolo-keras-master\\datasets\\VOC2007\\Annotations\\'
...
checkpoint = ModelCheckpoint('weights_zzx_res117_2.h5',
                             monitor='loss', 
                             verbose=1, 
                             save_best_only=True, 
                             mode='min', 
...
tb_counter  = len([log for log in os.listdir(os.path.expanduser('F:\\zzx\\YOLO\\yolo_resnet\\basic-yolo-keras-master\\logs\\zzx_yolo\\res\\')) if 'coco_zzx_yolo_117_' in log]) + 1
tensorboard = TensorBoard(log_dir=os.path.expanduser('F:\\zzx\\YOLO\\yolo_resnet\\basic-yolo-keras-master\\logs\\zzx_yolo\\res\\') + 'coco_zzx_yolo_117_' + '_' + str(tb_counter),
                          histogram_freq=0, 
                          write_graph=True, 
                          write_images=True)
...
model.compile(loss=custom_loss, optimizer=optimizer, metrics=['mae', 'acc', avg_rec])


model.load_weights("weights_zzx_res117_1.h5")
